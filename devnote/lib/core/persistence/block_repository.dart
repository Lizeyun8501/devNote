...
class SqliteBlockRepository implements BlockRepository {
  ...
  Future<List<BlockModel>> loadBlocks(String noteId) async {
    ...
    return persistenceBlocks.map(_toEditorBlock).toList();
  }
  ...
  Future<BlockModel> createBlock({
    ...
    return _toEditorBlock(persistenceBlock);
  }
  ...
  Future<BlockModel?> getBlock(String blockId) async {
    ...
    return persistenceBlock == null ? null : _toEditorBlock(persistenceBlock);
  }
  ...
  /// 将 persistence.BlockModel (FFI 返回，String blockType) 转换为 editor.BlockModel (BlockType enum)
  /// 保留 language/children/timestamps，避免双向转换字段丢失
  BlockModel _toEditorBlock(persistence.BlockModel block) {
    ...
    return BlockModel(
      ...
      children: List<String>.from(block.children),
      createdAt: block.createdAt ?? now,
      updatedAt: block.updatedAt ?? now,
    );
  }

  /// 将 editor.BlockModel (BlockType enum) 转换为 persistence.BlockModel (FFI 输入，String blockType)
  /// 保留 language/children/timestamps，避免双向转换字段丢失
  persistence.BlockModel _toCoreBlock(BlockModel block) {
    return persistence.BlockModel(
      id: block.id,
      noteId: block.noteId,
