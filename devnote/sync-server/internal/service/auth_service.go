...
//
// P0 修复（单实例状态）: SRP 会话从进程内 map 迁移到 StateStore，
// 支持多实例部署时跨进程共享会话。
...
// SRP 会话生命周期相关常量
const (
	// srpSessionTTL 单个 SRP 会话的最大有效时长，超过后 StateStore 自动过期。
...
//
// stateStore 用于持久化 SRP 会话，支持多实例部署。
func NewAuthService(db *sqlx.DB, cfg *config.Config, stateStore state.StateStore) *AuthService {
	return &AuthService{
		db:         db,
		cfg:        cfg,
...
func (s *AuthService) InitiateSRP(username string) (salt []byte, B []byte, err error) {
	...
	// 借鉴 1Password 的"会话隔离"：将 SRP 会话按 username 暂存到 StateStore，
	// 完成 VerifySRP 后立即删除（一次性会话）。
	// P0 修复: 序列化会话状态以支持多实例共享。
	sessionState := srv.ExportState()
	data, err := json.Marshal(sessionState)
	if err != nil {
		return nil, nil, fmt.Errorf("marshal SRP session: %w", err)
	}
	key := fmt.Sprintf("srp:session:%s", username)
	if err := s.stateStore.Set(context.Background(), key, string(data), srpSessionTTL); err != nil {
		return nil, nil, fmt.Errorf("store SRP session: %w", err)
	}
...
func (s *AuthService) VerifySRP(username string, A, M1 []byte) (M2 []byte, token string, err error) {
	...
	// 避免重放攻击。
	// P0 修复: 从 StateStore 读取并删除会话，支持多实例共享。
	key := fmt.Sprintf("srp:session:%s", username)
	raw, err := s.stateStore.Get(context.Background(), key)
	if err != nil {
		return nil, "", fmt.Errorf("load SRP session: %w", err)
	}
	if raw == "" {
		// 会话不存在或已过期（TTL 由 StateStore 管理）
		return nil, "", errors.New("no active SRP session")
	}
	// 立即删除，保证一次性会话
	s.stateStore.Del(context.Background(), key)

	var sessionState auth.ServerState
	if err := json.Unmarshal([]byte(raw), &sessionState); err != nil {
		return nil, "", fmt.Errorf("unmarshal SRP session: %w", err)
	}
	srv := auth.NewSRPServerFromState(s.srpParams, sessionState)
